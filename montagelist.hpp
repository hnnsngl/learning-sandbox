...
// display montage of images with classification labels for a dataset (to show false classifications)
                    ...
                    int tiles_per_row = 30)
{
	...
	if (list.size() == 0) return Mat(0,0,type);
	...
	for (int i = 0; i<list.size(); i++) {
		...
		std::string label = std::to_string(list[i].second) + "/" + std::to_string(dataset.labels[id]);
		
		cv::putText(mat, label, cv::Point(x, y + height - 2),
		            cv::FONT_HERSHEY_SIMPLEX, 0.4, cv::Scalar({0.0, 0.0, 0.0, 0.0}));
	}
	return mat;
}
